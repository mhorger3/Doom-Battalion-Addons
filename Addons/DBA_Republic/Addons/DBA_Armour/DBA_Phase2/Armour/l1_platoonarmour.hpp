
class Longbow_DBA_Uniform : DBA_P2cloneRecruitUniform
{
	displayName = "[DBA] Phase 2 Clone: Longbow 1 Trooper";
	hiddenSelectionsTextures[] =
	{
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\Longbow_Basic_Chest_CO.paa",
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\Longbow_Basic_Legs_CO.paa"
	};
	class ItemInfo : UniformItem
	{
		uniformModel = "-";
		uniformClass = "Longbow_DBA";
		containerClass = "supply150";
		uniformType = "Neopren";
		hiddenSelections[] =
		{
			"camo",
			"camo1"
		};
		mass = 40;
	};
	DBA_camoTemplate = "101st";
	DBA_camoType = "base";
	DBA_hasCamoChanger = 1;
};
class Longbow_Arid_DBA_Uniform : DBA_P2cloneRecruitUniform
{
	displayName = "[DBA] Phase 2 Clone: Longbow 1 Arid";
	hiddenSelectionsTextures[] =
	{
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\ARF_Arid_Chest_CO.paa",
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\ARF_Arid_Legs_CO.paa"
	};
	class ItemInfo : UniformItem
	{
		uniformModel = "-";
		uniformClass = "Longbow_Arid_DBA";
		containerClass = "supply150";
		uniformType = "Neopren";
		hiddenSelections[] =
		{
			"camo",
			"camo1"
		};
		mass = 40;
	};
	DBA_camoTemplate = "101st";
	DBA_camoType = "arid";
	DBA_hasCamoChanger = 1;
};
class Longbow_Recruit_DBA_Uniform : DBA_P2cloneRecruitUniform
{
	displayName = "[DBA] Phase 2 Clone: Longbow 1 Recruit";
	hiddenSelectionsTextures[] =
	{
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\ARF_Recruit_Top_CO.paa",
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\ARF_Recruit_Bottom_CO.paa"
	};
	class ItemInfo : UniformItem
	{
		uniformModel = "-";
		uniformClass = "Longbow_Recruit_DBA";
		containerClass = "supply150";
		uniformType = "Neopren";
		hiddenSelections[] =
		{
			"camo",
			"camo1"
		};
		mass = 40;
	};
	DBA_camoTemplate = "101st";
	DBA_camoType = "base";
	DBA_hasCamoChanger = 1;
};
class Longbow_Tropic_DBA_Uniform : DBA_P2cloneRecruitUniform
{
	displayName = "[DBA] Phase 2 Clone: Longbow 1 Tropic";
	hiddenSelectionsTextures[] =
	{
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\ARF_Tropic_Chest_CO.paa",
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\ARF_Tropic_Legs_CO.paa"
	};
	class ItemInfo : UniformItem
	{
		uniformModel = "-";
		uniformClass = "Longbow_Tropic_DBA";
		containerClass = "supply150";
		uniformType = "Neopren";
		hiddenSelections[] =
		{
			"camo",
			"camo1"
		};
		mass = 40;
	};
	DBA_camoTemplate = "101st";
	DBA_camoType = "tropic";
	DBA_hasCamoChanger = 1;
};
class Longbow_Night_DBA_Uniform : DBA_P2cloneRecruitUniform
{
	displayName = "[DBA] Phase 2 Clone: Longbow 1 Night";
	hiddenSelectionsTextures[] =
	{
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\ARF_Night_Chest_CO.paa",
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\ARF_Night_Legs_CO.paa"
	};
	class ItemInfo : UniformItem
	{
		uniformModel = "-";
		uniformClass = "Longbow_Night_DBA";
		containerClass = "supply150";
		uniformType = "Neopren";
		hiddenSelections[] =
		{
			"camo",
			"camo1"
		};
		mass = 40;
	};
	DBA_camoTemplate = "101st";
	DBA_camoType = "night";
	DBA_hasCamoChanger = 1;
};
class Longbow_Urban_DBA_Uniform : DBA_P2cloneRecruitUniform
{
	displayName = "[DBA] Phase 2 Clone: Longbow 1 Urban";
	hiddenSelectionsTextures[] =
	{
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\ARF_Urban_Chest_CO.paa",
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\ARF_Urban_Legs_CO.paa"
	};
	class ItemInfo : UniformItem
	{
		uniformModel = "-";
		uniformClass = "Longbow_Urban_DBA";
		containerClass = "supply150";
		uniformType = "Neopren";
		hiddenSelections[] =
		{
			"camo",
			"camo1"
		};
		mass = 40;
	};
	DBA_camoTemplate = "101st";
	DBA_camoType = "urban";
	DBA_hasCamoChanger = 1;
};
class Longbow_Winter_DBA_Uniform : DBA_P2cloneRecruitUniform
{
	displayName = "[DBA] Phase 2 Clone: Longbow 1 Winter";
	hiddenSelectionsTextures[] =
	{
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\ARF_Winter_Chest_CO.paa",
		"DBA_Republic\Addons\DBA_Armour\DBA_Phase2\Armour\data\Custom\Longbow\ARF_Winter_Legs_CO.paa"
	};
	class ItemInfo : UniformItem
	{
		uniformModel = "-";
		uniformClass = "Longbow_Winter_DBA";
		containerClass = "supply150";
		uniformType = "Neopren";
		hiddenSelections[] =
		{
			"camo",
			"camo1"
		};
		mass = 40;
	};
	DBA_camoTemplate = "101st";
	DBA_camoType = "winter";
	DBA_hasCamoChanger = 1;
};